...
export function NameSlide() {
  ...
  return (
    <div className="tools-section flex justify-center items-center gap-12 w-full py-8 overflow-hidden max-w-[1920px] mt-20">
        ...
        <div className="flex gap-2 animate-marquee group-hover:[animation-play-state:paused] whitespace-nowrap ">
          {[...names, ...names, ...names, ...names, ...names].map((n, i) => (
            <div key={i} className=" flex-shrink-0">
              <h1 className="name-slide font-porao text-secondary text-opacity-5">{n.name}</h1>
            </div>
          ))}
        </div>
      </div>
